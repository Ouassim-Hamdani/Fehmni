...
const NavBarLess = ({invisible,name,imageSource,nameNotification,session}) => {
  ...
  if(nameNotification === undefined || session === undefined){
    notif=<div style={{width: 250,display: 'flex',flexDirection:'column',alignItems: 'center', zIndex: 100, borderRadius:6,backgroundColor:'beige', marginTop:15, marginLeft: 900,paddingTop:15,paddingBottom:15}}>No notification.</div>
  }else{
    notif=<Notification nameNotification={nameNotification} session={session}/>
  }
