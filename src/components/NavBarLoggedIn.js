...
const NavBarLoggedIn = ({invisible,name,imageSource,nameNotification,session,support}) => {
  ...
  if(nameNotification === undefined || session === undefined){
    if (support==='mobile'){
      notif=<div style={{width: 150, zIndex: 100, borderRadius:6, marginTop:5, marginRight: 35, marginLeft: 'auto', backgroundColor: 'white'}}>No notification.</div>
    }
    else notif=<div style={{width: 250,display: 'flex',flexDirection:'column',alignItems: 'center', zIndex: 100, borderRadius:6,backgroundColor:'white', marginTop:15, marginLeft: 900,paddingTop:15,paddingBottom:15}}>No notification.</div>
  }else{
